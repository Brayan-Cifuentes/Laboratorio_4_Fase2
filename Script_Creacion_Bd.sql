...
create table Empleado_11113(
    ...
    foreign key (Codigo_Departamento) references Departamento_11113(Codigo_Departamento),
    foreign key (Codigo_Puesto) references Puesto_11113(Codigo_Puesto)
    
)Engine=InnoDB;


create table Nominae_11113(
	Codigo_Nomina varchar(5) primary key,
    Fecha_Inicial_Nomina date,
    Fecha_Final_Nomina date
)Engine=InnoDB;

create table Nominad_11113(
	Codigo_Nomina varchar(5),
    Codigo_Empleado varchar(5),
    Codigo_Concepto varchar(5),
    Valor_NominaD float,
    
    foreign key (Codigo_Nomina) references Nominae_11113(Codigo_Nomina),
    foreign key (Codigo_Empleado) references Empleado_11113(Codigo_Empleado),
    foreign key (Codigo_Concepto) references Concepto_11113(Codigo_Concepto)
